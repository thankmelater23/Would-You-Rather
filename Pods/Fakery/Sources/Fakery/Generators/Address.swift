...
extension Faker {
    public final class Address: Generator {
        ...
        public func coordinate(inRadius radius: Double, fromCenter center: Location) -> Location {
            ...
            #if swift(>=4.2)
                let rhoRandom = Double.random(in: 0 ..< Double.greatestFiniteMagnitude) / 0xFFFF_FFFF
                let phiRandom = Double.random(in: 0 ..< Double.greatestFiniteMagnitude) / 0xFFFF_FFFF
            #else
                let rhoRandom = Double(arc4random()) / 0xFFFF_FFFF
                let phiRandom = Double(arc4random()) / 0xFFFF_FFFF
            #endif
